...
CI_REGISTRY_IMAGE			?= ${CI_REGISTRY}/opensourcelab/py4owl
...
.build_${PYTHON_VERSION}: Dockerfile py4owl clean__py
	${_CMD_DOCKER_BUILD} \
...
test:	## run pytest on mounted-in tests (change tests, no rebuild!)
	...
	docker run \
			--platform linux/amd64 \
			--volume $(shell pwd)/bin/:/py4owl/bin/ \
			--volume $(shell pwd):/py4owl:rw \
			${DOCKER_IMAGE_TAG} \
			pytest --cov-report xml:/py4owl/bin/coverage.xml tests/
