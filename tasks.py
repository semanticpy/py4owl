...
SOURCE_DIR = ROOT_DIR.joinpath("py4owl")
...
PYPI_URL = "https://pypi.python.org/api/pypi/pypi/py4owl"
PYTHON_VERSION = 3.9
CI_PROJECT_NAME = "py4owl"
CI_REGISTRY_IMAGE = "registry.gitlab.com/opensourcelab/py4owl"
...
def docker_test(_c, python_version=PYTHON_VERSION):
    ...
    """
    volume_mount = (
        f"--volume {BIN_DIR}:/py4owl/bin/ --volume {ROOT_DIR}:/py4owl:rw"
    )
    registry_path = _get_registry_path_str(python_version)
    pytest_arg = f"pytest -v --cov-report xml:/py4owl/bin/coverage.xml {TEST_DIR}"
    Path(BIN_DIR).mkdir(parents=True, exist_ok=True)
